...
export class AdminPrefixlists implements OnInit, OnDestroy {
  ...
  filterButtonClass(mode: PrefixListFilterMode): string {
    ...
    if (isActive) {
      return 'rounded-lg border border-brand-300 bg-brand-50 px-3 py-2 text-xs font-semibold text-brand-800 shadow-theme-xs dark:border-brand-500/30 dark:bg-brand-500/15 dark:text-brand-300';
    }
